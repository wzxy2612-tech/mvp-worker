...
if load_cf_credentials; then
    ...
    if [ "$HTTP_CODE" = "200" ] \
       && [ "$(printf '%s' "$ROUTES_BODY" | jq -r '.success' 2>/dev/null)" = "true" ]; then
        ...
        log_error "雲端路由獲取失敗 (HTTP ${HTTP_CODE:-N/A})，跳過遠端路由備份。"
    fi
else
    log_info "未提供 Cloudflare 憑證，跳過遠端路由備份。"
fi
